...
This document specifies the gossip-based synchronization feature for BitChat, inspired by Plumtree. It ensures eventual consistency of public packets (ANNOUNCE and broadcast MESSAGE) across nodes via periodic sync requests containing a compact Golombâ€‘Coded Set (GCS) of recently seen packets.
...
- The receiver checks which packets are not in the senderâ€™s filter and sends those packets back. For announcements, only the latest announcement per peerID is sent; for broadcast messages, all missing ones are sent.
...
Implementation: `com.lemn.app.sync.PacketIdUtil`.

## GCS Filter (On-demand)

Implementation: `com.lemn.app.sync.GCSFilter`.

- Parameters (configurable):
  - size: 128â€“1024 bytes (default 256)
  - target false positive rate (FPR): default 1% (range 0.1%â€“5%)
- Derivations:
  - P = ceil(log2(1/FPR))
  - Maximum number of elements that fit into the filter is estimated as: N_max â‰ˆ floor((8 * sizeBytes) / (P + 2))
...
- What goes into the set:
  ...
  - Compute the 16-byte Packet ID (see below), then for hashing use the first 8 bytes of SHAâ€‘256 over the 16â€‘byte ID.
  - Map each hash to [0, M) with M = N * 2^P; sort ascending and encode deltas with Golombâ€‘Rice parameter P.

Hashing scheme (fixed for crossâ€‘impl compatibility):
- Packet ID: first 16 bytes of SHAâ€‘256 over [type | senderID | timestamp | payload].
- GCS hash: h64 = first 8 bytes of SHAâ€‘256 over the 16â€‘byte Packet ID, interpreted as an unsigned 64â€‘bit integer. Value = h64 % M.

## REQUEST_SYNC Packet

MessageType: `REQUEST_SYNC (0x21)`

- Header: normal BitChat header with TTL indicating â€œlocal-onlyâ€ semantics. Implementations SHOULD set TTL=0 to prevent any relay; neighbors still receive the packet over the direct link-layer. For periodic sync, recipient is broadcast; for per-peer initial sync, recipient is the specific peer.
- Payload: TLV with 16â€‘bit bigâ€‘endian length fields (type, length16, value)
  - 0x01: P (uint8) â€” Golombâ€‘Rice parameter
  - 0x02: M (uint32) â€” hash range N * 2^P
  - 0x03: data (opaque) â€” GCS bitstream (MSBâ€‘first bit packing)

Notes:
- The GCS bitstream uses MSBâ€‘first packing (bit 7 is the first bit in each byte).
- Receivers MUST reject filters with data length exceeding the local maximum (default 1024 bytes) to avoid DoS.

Encode/Decode implementation: `com.lemn.app.model.RequestSyncPacket`.

## Behavior

Sender behavior:
- Periodic: every 30 seconds, send REQUEST_SYNC with a freshly computed GCS snapshot, broadcast to immediate neighbors, and mark as localâ€‘only (TTL=0 recommended; do not relay).
...
- LEAVE handling: upon receiving a LEAVE message from a peer, immediately remove that peerâ€™s stored announcement from the sync candidate set.
- Stale/offline peer handling: when a peer is considered stale/offline (e.g., last announcement older than 60 seconds), immediately remove that peerâ€™s stored announcement from the sync candidate set.
...
- Identity announcements: `MessageType.ANNOUNCE`.
- Both packets produced by other peers and packets produced by the requester itself MUST be represented in the requesterâ€™s GCS; the responder MUST track and consider its own produced public packets as candidates to return when they are missing on the requester.
...
Exposed under â€œsync settingsâ€ in the debug settings sheet:
- Max packets per sync (default 100)
- Max GCS filter size in bytes (default 256, min 128, max 1024)
- GCS target FPR in percent (default 1%, 0.1%â€“5%)
...
- Packet ID recipe: first 16 bytes of SHAâ€‘256(type | senderID | timestamp | payload).
- GCS hashing function and mapping to [0, M) as specified above (v1), and MSBâ€‘first bit packing for the bitstream.
- Payload encoding: TLV with 16â€‘bit bigâ€‘endian lengths; TLV types 0x01 = P (uint8), 0x02 = M (uint32), 0x03 = data (opaque).
- Packet type and scope: REQUEST_SYNC = 0x21; local-only (not relayed); only ANNOUNCE and broadcast MESSAGE are synchronized; ANNOUNCE deâ€‘dupe is â€œlatest per sender peerIDâ€.

The following are requesterâ€‘defined and communicated or local policy (no global agreement required):
...
- Local storage policy: how many packets to consider and how you determine the â€œlatestâ€ announcement per peer.
...
- Practical bounds: data length in [0, 1024]; P in [1, 24]; M up to 2^32â€‘1.

Versioning:

- This document defines a fixed GCS hashing scheme (â€œv1â€) with no explicit version field in the payload. Changing the hashing or ID recipe would require a new message or an additional TLV in a future revision; current deployments must adhere to the constants above.
