...
import com.lemn.app.onboarding.BluetoothCheckScreen
import com.lemn.app.onboarding.BluetoothStatus
import com.lemn.app.onboarding.BluetoothStatusManager
import com.lemn.app.onboarding.BatteryOptimizationManager
import com.lemn.app.onboarding.BatteryOptimizationPreferenceManager
import com.lemn.app.onboarding.BatteryOptimizationScreen
import com.lemn.app.onboarding.BatteryOptimizationStatus
import com.lemn.app.onboarding.BackgroundLocationPermissionScreen
import com.lemn.app.onboarding.InitializationErrorScreen
import com.lemn.app.onboarding.InitializingScreen
import com.lemn.app.onboarding.LocationCheckScreen
import com.lemn.app.onboarding.LocationStatus
import com.lemn.app.onboarding.LocationStatusManager
import com.lemn.app.onboarding.OnboardingCoordinator
import com.lemn.app.onboarding.OnboardingState
import com.lemn.app.onboarding.PermissionExplanationScreen
import com.lemn.app.onboarding.PermissionManager
import com.lemn.app.ui.ChatScreen
import com.lemn.app.ui.ChatViewModel
import com.lemn.app.ui.OrientationAwareActivity
import com.lemn.app.ui.theme.BitchatTheme
import com.lemn.app.nostr.PoWPreferenceManager
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == com.lemn.app.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = android.content.IntentFilter(com.lemn.app.util.AppConstants.UI.ACTION_FORCE_FINISH)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                com.lemn.app.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        com.lemn.app.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        try { com.lemn.app.service.MeshForegroundService.start(applicationContext) } catch (_: Exception) { }
        meshService = com.lemn.app.service.MeshServiceHolder.getOrCreate(applicationContext)
        ...
        lifecycleScope.launch {
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                ...
                if (permissionManager.needsBackgroundLocationPermission() &&
                    ...
                    !com.lemn.app.onboarding.BackgroundLocationPreferenceManager.isSkipped(this@MainActivity)
                ) {
                    mainViewModel.updateOnboardingState(OnboardingState.BACKGROUND_LOCATION_EXPLANATION)
                } else {
    ...
    private fun initializeApp() {
        ...
        lifecycleScope.launch {
            try {
                ...
                com.lemn.app.nostr.LocationNotesInitializer.initialize(this@MainActivity)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.lemn.app.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            com.lemn.app.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
        
        when {
            shouldOpenPrivateChat -> {
                val peerID = intent.getStringExtra(com.lemn.app.ui.NotificationManager.EXTRA_PEER_ID)
                val senderNickname = intent.getStringExtra(com.lemn.app.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
            ...
            shouldOpenGeohashChat -> {
                val geohash = intent.getStringExtra(com.lemn.app.ui.NotificationManager.EXTRA_GEOHASH)
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> com.lemn.app.geohash.GeohashChannelLevel.BLOCK
                        6 -> com.lemn.app.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> com.lemn.app.geohash.GeohashChannelLevel.CITY
                        4 -> com.lemn.app.geohash.GeohashChannelLevel.PROVINCE
                        2 -> com.lemn.app.geohash.GeohashChannelLevel.REGION
                        else -> com.lemn.app.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = com.lemn.app.geohash.GeohashChannel(level, geohash)
                    val channelId = com.lemn.app.geohash.ChannelID.Location(geohashChannel)
