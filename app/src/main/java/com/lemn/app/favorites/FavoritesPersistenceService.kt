package com.lemn.app.favorites
...
/**
 * Manages favorites with Noiseâ†”Nostr mapping
...
class FavoritesPersistenceService private constructor(private val context: Context) {
    ...
    fun updateNostrPublicKeyForPeerID(peerID: String, nostrPubkey: String) {
        ...
        if (pid.length == 16 && pid.matches(Regex("^[0-9a-f]+$"))) {
            ...
            savePeerIdIndex()
            Log.d(TAG, "Indexed npub for peerID ${pid.take(8)}â€¦")
        } else {
    ...
    private fun loadPeerIdIndex() {
        try {
            ...
            if (json != null) {
                ...
                Log.d(TAG, "Loaded ${peerIdIndex.size} peerIDâ†’npub mappings")
            }
        } catch (e: Exception) {
    ...
    private fun savePeerIdIndex() {
        try {
            ...
            Log.d(TAG, "Saved ${peerIdIndex.size} peerIDâ†’npub mappings")
        } catch (e: Exception) {
    ...
    private fun normalizeNostrKeyToHex(value: String): String? = try {
        if (value.startsWith("npub1")) {
            val (hrp, data) = com.lemn.app.nostr.Bech32.decode(value)
