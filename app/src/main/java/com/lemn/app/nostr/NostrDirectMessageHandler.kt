...
import com.lemn.app.model.PrivateMessagePacket
import com.lemn.app.protocol.BitchatPacket
import com.lemn.app.services.SeenMessageStore
import com.lemn.app.ui.ChatState
import com.lemn.app.ui.MeshDelegateHandler
import com.lemn.app.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.lemn.app.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (packet.type != com.lemn.app.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.lemn.app.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                if (file != null) {
                    ...
                    val savedPath = com.lemn.app.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
