...
object RelayDirectory {
    ...
    private val httpClient: OkHttpClient
        get() = com.lemn.app.net.OkHttpProvider.httpClient()
    ...
    fun closestRelaysForGeohash(geohash: String, nRelays: Int): List<String> {
        ...
        val center = try {
            val c = com.lemn.app.geohash.Geohash.decodeToCenter(geohash)
            c
        } catch (e: Exception) {
    ...
    private fun fetchAndMaybeSwap(application: Application) {
        try {
            ...
            Log.i(TAG, "âœ… Using downloaded relay list (${dest.absolutePath}), entries=$entries, sha256=$hash, updatedAtMs=${getPrefs(application).getLong(KEY_LAST_UPDATE_MS, 0L)}")
        } catch (e: Exception) {
    ...
    private fun loadFromFile(file: File, sourceLabel: String): Boolean {
        return try {
            ...
            if (list.isEmpty()) {
                ...
                Log.i(TAG, "ðŸ“„ Loaded ${list.size} relay entries from ${sourceLabel} file (${file.absolutePath}), sha256=$hash")
                true
            }
        } catch (e: Exception) {
    ...
    private fun loadFromAssets(application: Application) {
        ...
        Log.i(TAG, "ðŸ“¦ Loaded ${list.size} relay entries from assets/$ASSET_FILE, sha256=$hash")
    }
