...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = com.lemn.app.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
    ...
    fun sendPrivateMessage(
        ...
        transportScope.launch {
            try {
                ...
                // Resolve by peerID first (new peerIDâ†’npub index), then fall back to noise key mapping
                ...
                val recipientPeerIDForEmbed = try {
                    com.lemn.app.favorites.FavoritesPersistenceService.shared
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                com.lemn.app.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is com.lemn.app.geohash.ChannelID.Location) {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.lemn.app.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            // 2) Legacy path: resolve by noise public key association
            val noiseKey = hexStringToByteArray(peerID)
            val favoriteStatus = com.lemn.app.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (peerID.length == 16) {
                val fallbackStatus = com.lemn.app.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
