...
object LocationNotesInitializer {
    ...
    fun initialize(context: Context): Boolean {
        return try {
            LocationNotesManager.getInstance().initialize(
                ...
                subscribe = { filter, id, handler ->
                    ...
                    val geohashFromFilter = filter.getGeohash() ?: run {
                        Log.e(TAG, "âŒ Cannot extract geohash from filter for location notes")
                        return@initialize id // Return subscription ID even on error
                    }
                    
                    Log.d(TAG, "ðŸ“ Location Notes subscribing to geohash: $geohashFromFilter")
            ...
            Log.d(TAG, "âœ… Location Notes Manager initialized")
            true
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Failed to initialize Location Notes Manager: ${e.message}", e)
            false
        }
    }
}
