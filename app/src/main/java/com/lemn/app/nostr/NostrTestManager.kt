...
class NostrTestManager(private val context: Context) {
     ...
     */
    fun runTests() {
        Log.i(TAG, "ðŸ§ª Starting Nostr functionality tests...")
        ...
        testScope.launch {
            try {
                ...
                Log.i(TAG, "âœ… All Nostr tests completed successfully!")
                
            } catch (e: Exception) {
                Log.e(TAG, "âŒ Nostr tests failed: ${e.message}", e)
            }
        }
    }
    ...
    private suspend fun testClientInitialization() {
        ...
        Log.d(TAG, "âœ… Client initialization successful")
    }
    ...
    private suspend fun testIdentityManagement() {
        ...
        Log.d(TAG, "âœ… Identity management test successful")
    }
    ...
    private suspend fun testRelayConnections() {
        ...
        Log.d(TAG, "âœ… Relay configuration test successful")
    }
    ...
    private suspend fun testCryptography() {
        ...
        Log.d(TAG, "âœ… Cryptography test successful")
    }
    ...
    private suspend fun testBech32() {
        ...
        Log.d(TAG, "âœ… Bech32 test successful")
    }
    ...
    private suspend fun testMessageSubscription() {
        ...
        nostrClient.subscribeToPrivateMessages { content, senderNpub, timestamp ->
            Log.d(TAG, "ðŸ“¥ Received test private message from $senderNpub: $content")
            messageReceived = true
        }
        
        // Subscribe to a test geohash
        nostrClient.subscribeToGeohash("u4pru") { content, senderPubkey, nickname, timestamp ->
            Log.d(TAG, "ðŸ“¥ Received test geohash message from ${senderPubkey.take(16)}...: $content")
            messageReceived = true
        }
        ...
        Log.d(TAG, "âœ… Message subscription test successful (no messages expected in test)")
    }
    ...
    fun testLoopbackMessage() {
        testScope.launch {
            try {
                ...
                Log.i(TAG, "ðŸ”„ Testing loopback private message...")
                ...
                nostrClient.sendPrivateMessage(
                    ...
                    onSuccess = {
                        Log.i(TAG, "âœ… Loopback message sent successfully")
                    },
                    onError = { error ->
                        Log.e(TAG, "âŒ Loopback message failed: $error")
                    }
                )
                
            } catch (e: Exception) {
                Log.e(TAG, "âŒ Loopback test failed: ${e.message}", e)
            }
        }
    }
    ...
    fun testGeohashMessage() {
        testScope.launch {
            try {
                Log.i(TAG, "ðŸŒ Testing geohash message...")
                ...
                nostrClient.sendGeohashMessage(
                    ...
                    onSuccess = {
                        Log.i(TAG, "âœ… Geohash message sent successfully")
                    },
                    onError = { error ->
                        Log.e(TAG, "âŒ Geohash message failed: $error")
                    }
                )
                
            } catch (e: Exception) {
                Log.e(TAG, "âŒ Geohash test failed: ${e.message}", e)
            }
        }
    }
    ...
    fun getDebugInfo(): String {
        return buildString {
            ...
            relays.forEach { relay ->
                appendLine("  ${relay.url}: ${if (relay.isConnected) "âœ…" else "âŒ"} (sent: ${relay.messagesSent}, received: ${relay.messagesReceived})")
            }
        }
    }
