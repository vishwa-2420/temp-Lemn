    ...
    fun decryptPrivateMessage(
        ...
        return try {
            ...
            val seal = unwrapGiftWrap(giftWrap, recipientIdentity.privateKeyHex)
                ?: run {
                    Log.w(TAG, "âŒ Failed to unwrap gift wrap")
                    return null
                }
            ...
            val rumor = openSeal(seal, recipientIdentity.privateKeyHex)
                ?: run {
                    Log.w(TAG, "âŒ Failed to open seal")
                    return null
                }
        ...
        if (powSettings.enabled && powSettings.difficulty > 0) {
            ...
            try {
                ...
                if (minedEvent != null) {
                    ...
                    Log.d(TAG, "âœ… PoW mining successful: target=${powSettings.difficulty}, actual=$actualDifficulty, nonce=${NostrProofOfWork.getNonce(event)}")
                } else {
                    Log.w(TAG, "âŒ PoW mining failed, proceeding without PoW")
                }
            } finally {
