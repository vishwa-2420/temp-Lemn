...
import com.lemn.app.model.BitchatFilePacket
...
class MediaSendingManager(
    ...
    companion object {
        ...
        private const val MAX_FILE_SIZE = com.lemn.app.util.AppConstants.Media.MAX_FILE_SIZE_BYTES // 50MB limit
    }
    ...
    fun sendVoiceNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            if (!file.exists()) {
                Log.e(TAG, "âŒ File does not exist: $filePath")
                return
            }
            Log.d(TAG, "ðŸ“ File exists: size=${file.length()} bytes, name=${file.name}")
            
            if (file.length() > MAX_FILE_SIZE) {
                Log.e(TAG, "âŒ File too large: ${file.length()} bytes (max: $MAX_FILE_SIZE)")
                return
            }
    ...
    fun sendImageNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            Log.e(TAG, "âŒ CRITICAL: Image send failed completely", e)
            Log.e(TAG, "âŒ Image path: $filePath")
            Log.e(TAG, "âŒ Error details: ${e.message}")
            Log.e(TAG, "âŒ Error type: ${e.javaClass.simpleName}")
        }
    }
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.lemn.app.features.file.FileUtils.getMimeTypeFromExtension(file.name) 
            } catch (_: Exception) { 
            ...
            Log.e(TAG, "âŒ CRITICAL: File send failed completely", e)
            Log.e(TAG, "âŒ File path: $filePath")
            Log.e(TAG, "âŒ Error details: ${e.message}")
            Log.e(TAG, "âŒ Error type: ${e.javaClass.simpleName}")
        }
    }
    ...
    private fun sendPrivateFile(
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.lemn.app.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
    ...
    private fun sendPublicFile(
        ...
        val payload = filePacket.encode()
        if (payload == null) {
            Log.e(TAG, "âŒ Failed to encode file packet for broadcast send")
            return
        }
     ...
     */
    fun handleTransferProgressEvent(evt: com.lemn.app.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.lemn.app.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.lemn.app.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
