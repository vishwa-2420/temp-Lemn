...
fun getRSSIColor(rssi: Int): Color {
    return when {
        rssi >= -50 -> Color(0xFF2F80ED) // Strong signal - blue
        rssi >= -60 -> Color(0xFF4DA3FF) // Good signal - light blue
        rssi >= -70 -> Color(0xFFF4C542) // Medium signal - yellow
        rssi >= -80 -> Color(0xFFB8BFC9) // Weak signal - muted
        else -> Color(0xFF8A93A0) // Very weak - subdued
    }
}
...
fun formatMessageHeaderAnnotatedString(
    ...
    if (message.sender != "system") {
        ...
        builder.append("  [${timeFormatter.format(message.timestamp)}]")
        message.powDifficulty?.let { bits ->
            if (bits > 0) builder.append(" â›¨${bits}b")
        }
        builder.pop()
    } else {
 ...
 * Avoids orange (~30Â°) reserved for self messages
 */
...
fun colorForPeerSeed(seed: String, isDark: Boolean): Color {
    ...
    // Avoid orange (~30Â°) reserved for self (matches iOS logic)
    ...
    for ((range, type) in allMatches) {
        ...
        when (type) {
            "mention" -> {
                ...
                val mentionColor = if (isMentionToMe) colorScheme.secondary else baseColor
