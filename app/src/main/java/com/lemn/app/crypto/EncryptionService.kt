...
import com.lemn.app.noise.NoiseEncryptionService
...
open class EncryptionService(private val context: Context) {
    ...
    fun signData(data: ByteArray): ByteArray? {
        return try {
            ...
            Log.d(TAG, "âœ… Generated Ed25519 signature (${signature.size} bytes)")
            signature
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Failed to sign data with Ed25519: ${e.message}")
            null
        }
    }
    ...
    fun clearPersistentIdentity() {
        ...
        try {
            ...
            Log.d(TAG, "ðŸ—‘ï¸ Cleared Ed25519 signing keys from preferences")

            // Generate new keys immediately
            val keyPair = loadOrCreateEd25519KeyPair()
            ed25519PrivateKey = keyPair.private as Ed25519PrivateKeyParameters
            ed25519PublicKey = keyPair.public as Ed25519PublicKeyParameters
            Log.d(TAG, "âœ… Rotated Ed25519 signing keys in memory")
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Failed to clear Ed25519 keys: ${e.message}")
        }
    }
    ...
    fun initiateHandshake(peerID: String): ByteArray? {
        Log.d(TAG, "ðŸ¤ Initiating Noise handshake with $peerID")
    ...
    fun processHandshakeMessage(data: ByteArray, peerID: String): ByteArray? {
        Log.d(TAG, "ðŸ¤ Processing handshake message from $peerID")
    ...
    fun removePeer(peerID: String) {
        ...
        Log.d(TAG, "ðŸ—‘ï¸ Removed session for $peerID")
    }
    ...
    fun initiateRekey(peerID: String): ByteArray? {
        Log.d(TAG, "ðŸ”„ Initiating rekey for $peerID")
    ...
    fun shutdown() {
        ...
        Log.d(TAG, "ðŸ”Œ EncryptionService shut down")
    }
    ...
    open fun verifyEd25519Signature(signature: ByteArray, data: ByteArray, publicKeyBytes: ByteArray): Boolean {
        return try {
            ...
            Log.d(TAG, "âœ… Ed25519 signature verification: $isValid")
            isValid
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Failed to verify Ed25519 signature: ${e.message}")
            false
        }
    }
    ...
    private fun loadOrCreateEd25519KeyPair(): AsymmetricCipherKeyPair {
        ...
        try {
            ...
            if (storedKey != null) {
                ...
                Log.d(TAG, "âœ… Loaded existing Ed25519 signing key pair")
                return AsymmetricCipherKeyPair(publicKey, privateKey)
            }
        } catch (e: Exception) {
            Log.w(TAG, "âš ï¸ Failed to load existing Ed25519 key, creating new one: ${e.message}")
        }
    ...
    fun generateAndSaveEd25519KeyPair(): AsymmetricCipherKeyPair {
        ...
        try {
            ...
            Log.d(TAG, "âœ… Created and stored new Ed25519 signing key pair")
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Failed to store Ed25519 private key: ${e.message}")
        }
    ...
    private fun migrateOldEd25519KeyIfNeeded() {
        try {
            ...
            if (oldKey != null && !prefs.contains(ED25519_PRIVATE_KEY_PREF)) {
                ...
                Log.d(TAG, "ðŸ” Migrated Ed25519 key to EncryptedSharedPreferences")
            }
        } catch (e: Exception) {
            Log.w(TAG, "âš ï¸ Failed to migrate Ed25519 key; generating new identity: ${e.message}")
        }
    }
}
