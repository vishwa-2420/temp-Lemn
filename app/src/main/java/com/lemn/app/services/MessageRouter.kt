...
import com.lemn.app.model.ReadReceipt
import com.lemn.app.nostr.NostrTransport
...
class MessageRouter private constructor(
            ...
            val instance = INSTANCE ?: synchronized(this) {
                INSTANCE ?: run {
                    ...
                    MessageRouter(context.applicationContext, mesh, nostr).also { instance ->
                        ...
                        try {
                            com.lemn.app.favorites.FavoritesPersistenceService.shared.addListener(instance.favoriteListener)
                        } catch (_: Exception) {}
                        INSTANCE = instance
                    }
                }
    ...
    private val favoriteListener = object: com.lemn.app.favorites.FavoritesChangeListener {
    ...
    fun sendPrivate(content: String, toPeerID: String, recipientNickname: String, messageID: String) {
        ...
        if (com.lemn.app.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            Log.d(TAG, "Routing PM via Nostr (geohash) to alias ${toPeerID.take(12)}â€¦ id=${messageID.take(8)}â€¦")
            val recipientHex = com.lemn.app.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                // Resolve the conversation's source geohash, so we can send from anywhere
                val sourceGeohash = com.lemn.app.nostr.GeohashConversationRegistry.get(toPeerID)
        ...
        if (hasMesh && hasEstablished) {
            Log.d(TAG, "Routing PM via mesh to ${toPeerID} msg_id=${messageID.take(8)}â€¦")
            mesh.sendPrivateMessage(content, toPeerID, recipientNickname, messageID)
        } else if (canSendViaNostr(toPeerID)) {
            Log.d(TAG, "Routing PM via Nostr to ${toPeerID.take(32)}â€¦ msg_id=${messageID.take(8)}â€¦")
            nostr.sendPrivateMessage(content, toPeerID, recipientNickname, messageID)
        } else {
            Log.d(TAG, "Queued PM for ${toPeerID} (no mesh, no Nostr mapping) msg_id=${messageID.take(8)}â€¦")
            val q = outbox.getOrPut(toPeerID) { mutableListOf() }
            q.add(Triple(content, recipientNickname, messageID))
            Log.d(TAG, "Initiating noise handshake after queueing PM for ${toPeerID.take(8)}â€¦")
    ...
    fun sendReadReceipt(receipt: ReadReceipt, toPeerID: String) {
        if ((mesh.getPeerInfo(toPeerID)?.isConnected == true) && mesh.hasEstablishedSession(toPeerID)) {
            Log.d(TAG, "Routing READ via mesh to ${toPeerID.take(8)}â€¦ id=${receipt.originalMessageID.take(8)}â€¦")
            mesh.sendReadReceipt(receipt.originalMessageID, toPeerID, mesh.getPeerNicknames()[toPeerID] ?: mesh.myPeerID)
        } else {
            Log.d(TAG, "Routing READ via Nostr to ${toPeerID.take(8)}â€¦ id=${receipt.originalMessageID.take(8)}â€¦")
    ...
    fun sendDeliveryAck(messageID: String, toPeerID: String) {
        ...
        if (com.lemn.app.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            val recipientHex = com.lemn.app.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                nostr.sendDeliveryAckGeohash(messageID, recipientHex, try { com.lemn.app.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)!! } catch (_: Exception) { return })
                return
            }
        }
        if (!((mesh.getPeerInfo(toPeerID)?.isConnected == true) && mesh.hasEstablishedSession(toPeerID))) {
    ...
    fun sendFavoriteNotification(toPeerID: String, isFavorite: Boolean) {
        if (mesh.getPeerInfo(toPeerID)?.isConnected == true) {
            val myNpub = try { com.lemn.app.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub } catch (_: Exception) { null }
    ...
    fun flushOutboxFor(peerID: String) {
        ...
        Log.d(TAG, "Flushing outbox for ${peerID.take(8)}â€¦ count=${queued.size}")
    ...
    private fun canSendViaNostr(peerID: String): Boolean {
        return try {
            ...
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                ...
                val fav = com.lemn.app.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                ...
                val fav = com.lemn.app.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
                fav?.isMutual == true && fav.peerNostrPublicKey != null
            } else {
                false
            }
