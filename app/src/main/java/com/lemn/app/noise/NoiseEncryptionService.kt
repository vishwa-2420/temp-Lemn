...
import com.lemn.app.mesh.PeerFingerprintManager
import com.lemn.app.noise.southernstorm.protocol.Noise
...
class NoiseEncryptionService(private val context: Context) {
    ...
    companion object {
        ...
        private const val REKEY_TIME_LIMIT = com.lemn.app.util.AppConstants.Noise.REKEY_TIME_LIMIT_MS // 1 hour (same as iOS)
        private const val REKEY_MESSAGE_LIMIT = com.lemn.app.util.AppConstants.Noise.REKEY_MESSAGE_LIMIT_ENCRYPTION // 1k messages (matches iOS) (same as iOS)
    }
    ...
    fun clearPersistentIdentity() {
        Log.w(TAG, "ðŸš¨ Panic Mode: Clearing persistent identity and rotating in-memory keys")
        ...
        Log.d(TAG, "âœ… Identity cleared and keys rotated")
    }
    ...
    private fun generateKeyPair(): Pair<ByteArray, ByteArray> {
        try {
            val dhState = com.lemn.app.noise.southernstorm.protocol.Noise.createDH("25519")
     ...
     */
    fun signPacket(packet: com.lemn.app.protocol.BitchatPacket): com.lemn.app.protocol.BitchatPacket? {
     ...
     */
    fun verifyPacketSignature(packet: com.lemn.app.protocol.BitchatPacket, publicKey: ByteArray): Boolean {
