package com.lemn.app.noise

import android.util.Log
import com.lemn.app.noise.southernstorm.protocol.*
...
class NoiseSession(
    ...
    companion object {
        ...
        private const val REKEY_TIME_LIMIT = com.lemn.app.util.AppConstants.Noise.REKEY_TIME_LIMIT_MS // 1 hour
        private const val REKEY_MESSAGE_LIMIT = com.lemn.app.util.AppConstants.Noise.REKEY_MESSAGE_LIMIT_SESSION // 10k messages
        ...
        private const val MAX_PAYLOAD_SIZE = com.lemn.app.util.AppConstants.Noise.MAX_PAYLOAD_SIZE_BYTES
        ...
        private const val HIGH_NONCE_WARNING_THRESHOLD = com.lemn.app.util.AppConstants.Noise.HIGH_NONCE_WARNING_THRESHOLD
        ...
        try {
            ...
            if (handshakeState?.needsLocalKeyPair() == true) {
                ...
                if (localKeyPair != null) {
                    ...
                    Log.d(TAG, "âœ“ Successfully set persistent static identity keys")
        ...
        try {
            ...
            return when (action) {
                ...
                HandshakeState.SPLIT -> {
                    ...
                    completeHandshake()
                    Log.d(TAG, "SPLIT âœ… XX handshake completed with $peerID")
                    null
                }
    ...
    private fun completeHandshake() {
        ...
        try {
            ...
            Log.d(TAG, "âœ… XX handshake completed with $peerID")
        } catch (e: Exception) {
            state = NoiseSessionState.Failed(e)
    ...
    fun encrypt(data: ByteArray): ByteArray {
        ...
        synchronized(cipherLock) {
            ...
            try {
                ...
                Log.d(TAG, "âœ… ANDROID ENCRYPT: ${data.size} â†’ ${combinedPayload.size} bytes (nonce: $currentNonce, ciphertextLength+TAG: ${ciphertextLength}) for $peerID (msg #$messagesSent, role: ${if (isInitiator) "INITIATOR" else "RESPONDER"})")
    ...
    fun decrypt(combinedPayload: ByteArray): ByteArray {
        ...
        synchronized(cipherLock) {
            ...
            try {
                ...
                Log.d(TAG, "âœ… ANDROID DECRYPT: ${combinedPayload.size} â†’ ${result.size} bytes from $peerID (nonce: $extractedNonce, highest: $highestReceivedNonce, role: ${if (isInitiator) "INITIATOR" else "RESPONDER"})")
                return result
