...
import com.lemn.app.model.FragmentPayload
...
class FragmentManager {
    ...
    companion object {
        ...
        private const val FRAGMENT_SIZE_THRESHOLD = com.lemn.app.util.AppConstants.Fragmentation.FRAGMENT_SIZE_THRESHOLD // Matches iOS: if data.count > 512
        private const val MAX_FRAGMENT_SIZE = com.lemn.app.util.AppConstants.Fragmentation.MAX_FRAGMENT_SIZE        // Matches iOS: maxFragmentSize = 469 
        private const val FRAGMENT_TIMEOUT = com.lemn.app.util.AppConstants.Fragmentation.FRAGMENT_TIMEOUT_MS     // Matches iOS: 30 seconds cleanup
        private const val CLEANUP_INTERVAL = com.lemn.app.util.AppConstants.Fragmentation.CLEANUP_INTERVAL_MS     // 10 seconds cleanup check
    }
    ...
    fun createFragments(packet: BitchatPacket): List<BitchatPacket> {
        ...
        val encoded = packet.toBinaryData()
            if (encoded == null) {
                Log.e(TAG, "âŒ Failed to encode packet to binary data")
                return emptyList()
            }
        ...
        val fullData = try {
                ...
                Log.e(TAG, "âŒ Failed to unpad data: ${e.message}", e)
                return emptyList()
            }
        ...
        if (maxDataSize <= 0) {
            Log.e(TAG, "âŒ Calculated maxDataSize is non-positive ($maxDataSize). Route too large?")
            return emptyList()
        }
        ...
        Log.d(TAG, "âœ… Created ${fragments.size} fragments successfully")
            ...
            Log.e(TAG, "âŒ Fragment creation failed: ${e.message}", e)
            Log.e(TAG, "âŒ Packet type: ${packet.type}, payload: ${packet.payload.size} bytes")
            return emptyList()
        }
    }
