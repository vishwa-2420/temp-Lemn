...
class BluetoothPacketBroadcaster(
    ...
    companion object {
        ...
        private const val CLEANUP_DELAY = com.lemn.app.util.AppConstants.Mesh.BROADCAST_CLEANUP_DELAY_MS
    }
    ...
    private fun logPacketRelay(
        ...
        try {
            ...
            val manager = com.lemn.app.ui.debug.DebugSettingsManager.getInstance()
    ...
    fun broadcastPacket(
        ...
        if (fragmentManager != null) {
            val fragments = try {
                ...
                if (isFile) {
                    Log.e(TAG, "âŒ File fragmentation failed for ${packet.payload.size} byte file")
                }
                return
            }
            if (fragments.size > 1) {
