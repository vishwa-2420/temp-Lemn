...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    companion object {
        ...
        private const val MESSAGE_TIMEOUT = com.lemn.app.util.AppConstants.Security.MESSAGE_TIMEOUT_MS // 5 minutes (same as iOS)
        private const val CLEANUP_INTERVAL = com.lemn.app.util.AppConstants.Security.CLEANUP_INTERVAL_MS // 5 minutes
        private const val MAX_PROCESSED_MESSAGES = com.lemn.app.util.AppConstants.Security.MAX_PROCESSED_MESSAGES
        private const val MAX_PROCESSED_KEY_EXCHANGES = com.lemn.app.util.AppConstants.Security.MAX_PROCESSED_KEY_EXCHANGES
    }
    ...
    fun validatePacket(packet: BitchatPacket, peerID: String): Boolean {
        ...
        if (processedMessages.contains(messageID)) {
            ...
            val isFreshAnnounce = messageType == MessageType.ANNOUNCE &&
                    packet.ttl >= com.lemn.app.util.AppConstants.MESSAGE_TTL_HOPS
    ...
    private fun verifyPacketSignature(packet: BitchatPacket, peerID: String): Boolean {
        try {
            ...
            if (packet.signature == null) {
                Log.w(TAG, "âŒ Signature check for $peerID: NO_SIGNATURE (packet type ${packet.type})")
                return false
            }
            ...
            if (MessageType.fromValue(packet.type) == MessageType.ANNOUNCE) {
                ...
                try {
                    val announcement = com.lemn.app.model.IdentityAnnouncement.decode(packet.payload)
            ...
            if (signingPublicKey == null) {
                ...
                Log.w(TAG, "âŒ Signature check for $peerID: NO_SIGNING_KEY_AVAILABLE (packet type ${packet.type})")
                return false
            }
            ...
            if (packetDataForSigning == null) {
                Log.w(TAG, "âŒ Signature check for $peerID: ENCODING_ERROR (packet type ${packet.type})")
                return false
            }
            ...
            if (isSignatureValid) {
                // Log.v(TAG, "âœ… Signature verified for $peerID (type ${packet.type})")
                return true
            } else {
                Log.w(TAG, "âŒ Signature INVALID for $peerID (type ${packet.type})")
                return false
            }
            
        } catch (e: Exception) {
            Log.e(TAG, "âŒ Signature verification error for $peerID: ${e.message}")
            return false
        }
    }
