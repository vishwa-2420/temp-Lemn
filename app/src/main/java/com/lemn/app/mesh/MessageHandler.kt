...
import com.lemn.app.model.BitchatMessageType
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        if (recipientID != myPeerID) {
            Log.d(TAG, "ðŸ” Encrypted message not for me (for $recipientID, I am $myPeerID)")
            return
        }
        ...
        try {
            ...
            val noisePayload = com.lemn.app.model.NoisePayload.decode(decryptedData)
            if (noisePayload == null) {
                Log.w(TAG, "Failed to parse NoisePayload from $peerID")
                return
            }
            
            Log.d(TAG, "ðŸ”“ Decrypted NoisePayload type ${noisePayload.type} from $peerID")
            
            when (noisePayload.type) {
                com.lemn.app.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.lemn.app.model.PrivateMessagePacket.decode(noisePayload.data)
                    if (privateMessage != null) {
                        Log.d(TAG, "ðŸ”“ Decrypted TLV PM from $peerID: ${privateMessage.content.take(30)}...")
                ...
                com.lemn.app.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.lemn.app.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "ðŸ”“ Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.lemn.app.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.lemn.app.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                        ...
                        Log.d(TAG, "ðŸ“„ Saved encrypted incoming file to $savedPath (msgId=$uniqueMsgId)")
                        delegate?.onMessageReceived(message)
                ...
                com.lemn.app.model.NoisePayloadType.DELIVERED -> {
                    // Handle delivery ACK exactly like iOS
                    val messageID = String(noisePayload.data, Charsets.UTF_8)
                    Log.d(TAG, "ðŸ“¬ Delivery ACK received from $peerID for message $messageID")
                    
                    // Simplified: Call delegate with messageID and peerID directly
                    delegate?.onDeliveryAckReceived(messageID, peerID)
                }
                
                com.lemn.app.model.NoisePayloadType.READ_RECEIPT -> {
                    // Handle read receipt exactly like iOS
                    val messageID = String(noisePayload.data, Charsets.UTF_8)
                    Log.d(TAG, "ðŸ‘ï¸ Read receipt received from $peerID for message $messageID")
                    
                    // Simplified: Call delegate with messageID and peerID directly
                    delegate?.onReadReceiptReceived(messageID, peerID)
                }
                com.lemn.app.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                    Log.d(TAG, "ðŸ” Verify challenge received from $peerID (${noisePayload.data.size} bytes)")
                    delegate?.onVerifyChallengeReceived(peerID, noisePayload.data, packet.timestamp.toLong())
                }
                com.lemn.app.model.NoisePayloadType.VERIFY_RESPONSE -> {
                    Log.d(TAG, "ðŸ” Verify response received from $peerID (${noisePayload.data.size} bytes)")
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.lemn.app.model.NoisePayload(
                type = com.lemn.app.model.NoisePayloadType.DELIVERED,
            ...
            Log.d(TAG, "ðŸ“¤ Sent delivery ACK to $senderPeerID for message $messageID")
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (age > com.lemn.app.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
            Log.w(TAG, "Ignoring stale ANNOUNCE from ${peerID.take(8)} (age=${age}ms > ${com.lemn.app.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
            return false
        }
        ...
        if (!verified) {
            Log.w(TAG, "âŒ Ignoring unverified announce from ${peerID.take(8)}...")
            return false
        }
        
        // Successfully decoded TLV format exactly like iOS
        Log.d(TAG, "âœ… Verified announce from $peerID: nickname=${announcement.nickname}, " +
        ...
        try {
            val neighborsOrNull = com.lemn.app.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = BitchatPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.lemn.app.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
                
                delegate?.sendPacket(responsePacket)
                Log.d(TAG, "ðŸ“¤ Sent handshake response to $peerID")
            }
            
            // Check if session is now established
            val hasSession = delegate?.hasNoiseSession(peerID) ?: false
            if (hasSession) {
                Log.d(TAG, "âœ… Noise session established with $peerID")
            }
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        if (peerInfo == null || !peerInfo.isVerifiedNickname) {
            Log.w(TAG, "ðŸš« Dropping public message from unverified or unknown peer ${peerID.take(8)}...")
            return
        }
        
        try {
            // Try file packet first (voice, image, etc.) and log outcome for FILE_TRANSFER
            val isFileTransfer = com.lemn.app.protocol.MessageType.fromValue(packet.type) == com.lemn.app.protocol.MessageType.FILE_TRANSFER
            val file = com.lemn.app.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
                    Log.d(TAG, "ðŸ“¥ FILE_TRANSFER decode success (broadcast): name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}', from=${peerID.take(8)}")
                }
                val savedPath = com.lemn.app.features.file.FileUtils.saveIncomingFile(appContext, file)
                val message = BitchatMessage(
                ...
                Log.d(TAG, "ðŸ“„ Saved incoming file to $savedPath")
                delegate?.onMessageReceived(message)
                return
    ...
    private suspend fun handlePrivateMessage(packet: BitchatPacket, peerID: String) {
        try {
            ...
            if (file != null) {
                if (isFileTransfer) {
                    Log.d(TAG, "ðŸ“¥ FILE_TRANSFER decode success (private): name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}', from=${peerID.take(8)}")
                }
                val savedPath = com.lemn.app.features.file.FileUtils.saveIncomingFile(appContext, file)
                val message = BitchatMessage(
                    id = java.util.UUID.randomUUID().toString().uppercase(),
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.lemn.app.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.lemn.app.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.lemn.app.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }

                // Determine iOS-style guidance text
                val rel = com.lemn.app.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                ...
                val sys = com.lemn.app.model.BitchatMessage(
                    sender = "system",
