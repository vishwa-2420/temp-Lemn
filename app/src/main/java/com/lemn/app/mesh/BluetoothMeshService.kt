package com.lemn.app.mesh

import android.content.Context
import android.util.Log
import com.lemn.app.crypto.EncryptionService
import com.lemn.app.model.BitchatMessage
import com.lemn.app.protocol.MessagePadding
import com.lemn.app.model.RoutedPacket
import com.lemn.app.model.IdentityAnnouncement
import com.lemn.app.model.NoisePayload
import com.lemn.app.model.NoisePayloadType
import com.lemn.app.protocol.BitchatPacket
import com.lemn.app.protocol.MessageType
import com.lemn.app.protocol.SpecialRecipients
import com.lemn.app.model.RequestSyncPacket
import com.lemn.app.sync.GossipSyncManager
import com.lemn.app.util.toHexString
import com.lemn.app.services.VerificationService
...
class BluetoothMeshService(private val context: Context) {
    private val debugManager by lazy { try { com.lemn.app.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }
    
    companion object {
        private const val TAG = "BluetoothMeshService"
        private val MAX_TTL: UByte = com.lemn.app.util.AppConstants.MESSAGE_TTL_HOPS
    }
    ...
    private val serviceNotificationManager = com.lemn.app.ui.NotificationManager(
        context.applicationContext,
        androidx.core.app.NotificationManagerCompat.from(context.applicationContext),
        com.lemn.app.util.NotificationIntervalManager()
    )
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.lemn.app.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.lemn.app.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.lemn.app.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.lemn.app.services.AppStateStore.setPeers(peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.lemn.app.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.lemn.app.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.lemn.app.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
                ...
                try {
                    when {
                        message.isPrivate -> {
                            ...
                            if (peer.isNotEmpty()) com.lemn.app.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.lemn.app.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.lemn.app.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                if (delegate == null && message.isPrivate) {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = com.lemn.app.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
            ...
            override fun handleAnnounce(routed: RoutedPacket) {
                serviceScope.launch {
                    ...
                    if (deviceAddress != null && pid != null) {
                        ...
                        val isDirect = routed.packet.ttl == com.lemn.app.util.AppConstants.MESSAGE_TTL_HOPS
        ...
        override fun onPacketReceived(packet: BitchatPacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
            ...
            try {
                com.lemn.app.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                    packet = packet,
                    fromPeerID = peerID,
                    fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.lemn.app.ui.debug.DebugSettingsManager.getInstance()
     ...
     */
    fun sendFileBroadcast(file: com.lemn.app.model.BitchatFilePacket) {
        try {
            Log.d(TAG, "ðŸ“¤ sendFileBroadcast: name=${file.fileName}, size=${file.fileSize}")
            val payload = file.encode()
            if (payload == null) {
                Log.e(TAG, "âŒ Failed to encode file packet in sendFileBroadcast")
                return
            }
        ...
        serviceScope.launch {
            ...
            Log.e(TAG, "âŒ sendFileBroadcast failed: ${e.message}", e)
            Log.e(TAG, "âŒ File: name=${file.fileName}, size=${file.fileSize}")
        }
    }
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: com.lemn.app.model.BitchatFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        if (filePayload == null) {
                            Log.e(TAG, "âŒ Failed to encode file packet for private send")
                            return@launch
                        }
                        Log.d(TAG, "ðŸ“¦ Encoded file TLV: ${filePayload.size} bytes")
                        
                        // Create NoisePayload wrapper (type byte + file TLV data) - same as iOS
                        val noisePayload = com.lemn.app.model.NoisePayload(
                            type = com.lemn.app.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
                        ...
                        if (encrypted == null) {
                            Log.e(TAG, "âŒ Failed to encrypt file for $recipientPeerID")
                            return@launch
                        }
                        ...
                        val packet = BitchatPacket(
                            ...
                            payload = encrypted,
                            signature = null,
                            ttl = com.lemn.app.util.AppConstants.MESSAGE_TTL_HOPS
                        )
                        ...
                        Log.e(TAG, "âŒ Failed to encrypt file for $recipientPeerID: ${e.message}", e)
                    }
                } else {
            ...
            Log.e(TAG, "âŒ sendFilePrivate failed: ${e.message}", e)
            Log.e(TAG, "âŒ File: to=$recipientPeerID, name=${file.fileName}, size=${file.fileSize}")
        }
    }
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.lemn.app.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.lemn.app.model.NoisePayload(
                        type = com.lemn.app.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.lemn.app.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.lemn.app.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.lemn.app.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Avoid duplicate read receipts: check persistent store first
                val seenStore = try { com.lemn.app.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.lemn.app.model.NoisePayload(
                    type = com.lemn.app.model.NoisePayloadType.READ_RECEIPT,
                ...
                val packet = BitchatPacket(
                    ...
                    payload = encrypted,
                    signature = null,
                    ttl = com.lemn.app.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS messageTTL
                )
    ...
    private fun sendNoisePayloadToPeer(payload: NoisePayload, recipientPeerID: String, label: String) {
        serviceScope.launch {
            try {
                ...
                val signedPacket = signPacketBeforeBroadcast(packet)
                connectionManager.broadcastPacket(RoutedPacket(signedPacket))
                Log.d(TAG, "ðŸ“¤ Sent $label to $recipientPeerID (${payload.data.size} bytes)")
            } catch (e: Exception) {
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            val nickname = try { com.lemn.app.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
            ...
            try {
                ...
                if (directPeers.isNotEmpty()) {
                    val gossip = com.lemn.app.services.meshgraph.GossipTLV.encodeNeighbors(directPeers)
                    tlvPayload = tlvPayload + gossip
                }
                // Always update our own node in the mesh graph with the neighbor list we used
                try {
                    com.lemn.app.services.meshgraph.MeshGraphService.getInstance()
     ...
     */
    fun getSessionState(peerID: String): com.lemn.app.noise.NoiseSession.NoiseSessionState {
        ...
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.lemn.app.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
    ...
    fun clearAllInternalData() {
        ...
        try {
            ...
            Log.e(TAG, "âŒ Error clearing mesh service internal data: ${e.message}")
        }
    }
    ...
    fun clearAllEncryptionData() {
        ...
        try {
            ...
            Log.e(TAG, "âŒ Error clearing encryption data: ${e.message}")
        }
    }
}
