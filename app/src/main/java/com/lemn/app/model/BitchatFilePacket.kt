 ...
 *  - 0x04: content (bytes) â€” may appear multiple times for large files
 *
...
data class BitchatFilePacket(
    ...
    fun encode(): ByteArray? {
        try {
            android.util.Log.d("BitchatFilePacket", "ðŸ”„ Encoding: name=$fileName, size=$fileSize, mime=$mimeType")
        ...
        if (nameBytes.size > 0xFFFF || mimeBytes.size > 0xFFFF) {
                android.util.Log.e("BitchatFilePacket", "âŒ TLV field too large: name=${nameBytes.size}, mime=${mimeBytes.size} (max: 65535)")
                return null
            }
            if (content.size > 0xFFFF) {
                android.util.Log.d("BitchatFilePacket", "ðŸ“¦ Content exceeds 65535 bytes (${content.size}); will be split into multiple CONTENT TLVs")
            } else {
                android.util.Log.d("BitchatFilePacket", "ðŸ“ TLV sizes OK: name=${nameBytes.size}, mime=${mimeBytes.size}, content=${content.size}")
            }
        ...
        val result = buf.array()
            android.util.Log.d("BitchatFilePacket", "âœ… Encoded successfully: ${result.size} bytes total")
            return result
        } catch (e: Exception) {
            android.util.Log.e("BitchatFilePacket", "âŒ Encoding failed: ${e.message}", e)
            return null
        }
    }
    ...
    companion object {
        fun decode(data: ByteArray): BitchatFilePacket? {
            android.util.Log.d("BitchatFilePacket", "ðŸ”„ Decoding ${data.size} bytes")
            try {
                var off = 0
                ...
                android.util.Log.d("BitchatFilePacket", "âœ… Decoded: name=$n, size=$s, mime=$m, content=${c.size} bytes")
                return result
            } catch (e: Exception) {
                android.util.Log.e("BitchatFilePacket", "âŒ Decoding failed: ${e.message}", e)
                return null
            }
        }
    }
